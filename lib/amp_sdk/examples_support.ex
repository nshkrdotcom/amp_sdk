defmodule AmpSdk.ExamplesSupport do
  @moduledoc false

  alias AmpSdk.CLIInvoke
  ...
  @spec invoke([String.t()], keyword()) :: {:ok, String.t()} | {:error, AmpSdk.Error.t()}
  def invoke(args, opts \\ []) when is_list(args) and is_list(opts) do
    CLIInvoke.invoke(args, command_opts(opts))
  end
  ...
  defp maybe_put_danger_full_access(options) when is_map(options) do
    if danger_full_access?() do
      Map.put(options, :dangerously_allow_all, true)
    else
      options
    end
  end
  ...
  defp maybe_put_command_danger_full_access(opts) when is_list(opts) do
    if danger_full_access?() do
      Keyword.put(opts, :dangerously_allow_all, true)
    else
      opts
    end
  end
